 ...
 */
class NowPlayingDataSource(private val repo: MovieRepository, private val scope: CoroutineScope) : PageKeyedDataSource<Int, MovieData>() {
    private val page = 1
    ...
    override fun loadInitial(params: LoadInitialParams<Int>, callback: LoadInitialCallback<Int, MovieData>) {
        ...
        scope.launch {
            try {
                val result = repo.getNowPlayingMoviePagingAsync(page).results
    ...
    override fun loadAfter(params: LoadParams<Int>, callback: LoadCallback<Int, MovieData>) {
        scope.launch {
            try {
                val result = repo.getNowPlayingMoviePagingAsync(params.key).results
                if (result.isNotEmpty()) {
